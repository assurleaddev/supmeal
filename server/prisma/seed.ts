import { PrismaClient, TagType, CookbookRole, MealType } from '@prisma/client';
...
async function main() {
  ...
  const bob = await prisma.user.upsert({
    ...
    create: {
      ...
      username: 'bob',
      passwordHash: await hash('password123'),
      preferences: { create: { diet: [], allergies: ['noix'], cuisineTypes: ['française'], defaultPortions: 2 } },
    },
  });
  ...
  const recipes: RecipeSeed[] = [
    ...
    // ── Soupe à l'oignon ─────────────────────────────────────────────────────
    {
      ...
      steps: [
        ...
        { description: 'Ajouter les oignons, couvrir et cuire 20 min à feu doux en remuant régulièrement. Ils doivent devenir translucides.', duration: 20 },
        { description: 'Retirer le couvercle, monter à feu moyen et laisser caraméliser 20 minutes supplémentaires jusqu\'à coloration dorée foncée. Remuer souvent.', duration: 20 },
    ...
    // ── Tarte Tatin ──────────────────────────────────────────────────────────
    {
      ...
      steps: [
        ...
        { description: 'Dans une poêle allant au four (ou moule à manqué), faire fondre le beurre à feu moyen. Ajouter le sucre et laisser caraméliser sans remuer jusqu\'à obtenir un caramel blond ambré.', duration: 8 },
        { description: 'Disposer les quartiers de pommes bien serrés en rosace dans le caramel. Cuire 10 minutes à feu moyen pour compoter légèrement.', duration: 10 },
  ...
  for (const r of recipes) {
    // Upsert each ingredient and collect ids
    const ingredientData: { ingredientId: string; quantity?: number; unit?: string; notes?: string; orderIndex: number }[] = [];
    ...
    // Upsert recipe (delete dependents first on conflict to allow re-seed)
    ...
    await prisma.recipe.upsert({
      ...
      create: {
        ...
        tags: {
          create: r.tags.map((name) => ({ tagId: tagMap[name] })).filter((t) => t.tagId),
        },
      },
    });
  ...
  // ── 8. Comments ────────────────────────────────────────────────────────────
  const existingComments = await prisma.comment.count({ where: { recipeId: { in: ['seed-recipe-carbonara', 'seed-recipe-quiche', 'seed-recipe-ratatouille'] } } });
